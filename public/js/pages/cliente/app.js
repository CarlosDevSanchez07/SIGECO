$(function() {
    ...
    $('#formClientes').validate({
      ...
      */
     $('#formClientes input[type=text]').on('keyup',function(e){
      ...
      */
    $('#addClientes').on('click', function(e){
        e.preventDefault()
        proccesData('add')
    })

    $('#modCliente').on('click', function(e){
        e.preventDefault()
    ...
    $('.tableAccion').on('click', function(e){
        e.preventDefault()
        $('#addClientes').addClass('hidden')
        $('#modCliente').addClass('hidden')
        let id = $(this).attr('data-id')
        let baseUrl = $(location).attr('href')
        baseUrl = baseUrl.split("/")
        baseUrl[4] = 'getCliente'
        baseUrl[5] = id
    ...
    $('#insertCliente').on('click',function(){
        $('#addClientes').removeClass('hidden')
        $('#modCliente').addClass('hidden')
    })
    ...
    function viewDataCliente(res){
        ...
        $('[name='+key[12]+'][value='+res[12]+']').attr('checked', 'true')
        $('[name='+key[13]+'][value='+res[13]+']').attr('checked', 'true')
    ...
    function proccesData(type, id = ''){
        let baseUrl = $(location).attr('href')
        baseUrl = baseUrl.split("/")
        baseUrl[4] = type == 'add' ? 'addCliente' : 'updateCliente'
        if(type == 'update')
            baseUrl[5] = id
        baseUrl = baseUrl.join("/")
        let dataForm = JSON.stringify($('#formClientes').serializeArray())
        $.ajax({
            url : baseUrl,
            method : $('#formClientes').attr("method"),
            data : {'data' : dataForm},
            success : function(res){
