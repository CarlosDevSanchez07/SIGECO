$(function() {
    ...
    $('#formEmpleado').validate({
      ...
      */
     $('#formEmpleado input[type=text]').on('keyup',function(e){
         $('input[name='+e.target.name+']').val($('input[name='+e.target.name+']').val().toUpperCase())
     })

     /**
      * funcion para obtener los datos del modal que proporciona los datos del empleado para el ingreso a la base de datos
      */
    ...
    $('#modEmpleado').on('click', function(e){
        e.preventDefault()
    ...
    $('.tableAccion').on('click', function(e){
        e.preventDefault()
        $('#addEmpleado').addClass('hidden')
        $('#modEmpleado').addClass('hidden')
        let id = $(this).attr('data-id')
        let baseUrl = $(location).attr('href')
        baseUrl = baseUrl.split("/")
        baseUrl[4] = 'getEmpleado'
        baseUrl[5] = id
        ...
        $.ajax({
            ...
            success : function(res){
                res = JSON.parse(res)
                viewDataEmpleado(res)
            }
        })
    })

    $('#insertEmpleado').on('click',function(){
        $('#addEmpleado').removeClass('hidden')
        $('#modEmpleado').addClass('hidden')
    })
    ...
    $('.accionMod').on('click',function(e){
        e.preventDefault()
        $('#addEmpleado').addClass('hidden')
        $('#modEmpleado').removeClass('hidden')
        let id = $(this).attr('data-id')
        let baseUrl = $(location).attr('href')
        baseUrl = baseUrl.split("/")
        baseUrl[4] = 'getEmpleado'
    ...
    function proccesData(type, id = ''){
        let baseUrl = $(location).attr('href')
        baseUrl = baseUrl.split("/")
        baseUrl[4] = type == 'add' ? 'addEmpleado' : 'updateEmpleado'
        if(type == 'update')
            baseUrl[5] = id
        baseUrl = baseUrl.join("/")
        let dataForm = JSON.stringify($('#formEmpleado').serializeArray())
        $.ajax({
            url : baseUrl,
            method : $('#formEmpleado').attr("method"),
            data : {'data' : dataForm},
            success : function(res){
